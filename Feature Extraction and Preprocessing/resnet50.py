...
paths={'saved_dire':join('datasets_features','smarthome','rgb','densenet101','CS','test'),
'rgb_raw':join('raw_dataset','CS','test'),
'json_path':join('datasets_features','smarthome','new_pose','CS','test')
...
def features_extraction_model(resized_height=300,resized_width=300):
...
def extract_features(video_dire): ## takes the directory of the video as input 
    ...
    frames=sample_frames(video_dire,300,300)
    ...
    if len_features < seq_len:
        ...
        padd_arr=np.zeros((diff_frame,features_siz),dtype=np.float32)
